...
public class Ets6ProjectLoader : BaseProjectLoader
{
    ...
    public Ets6ProjectLoader(ILogger<Ets6ProjectLoader> logger) : base(logger) { }
        ...
        CancellationToken cancellationToken)
    {
        ...
        // ETS6: Segment-Tag zwischen Line und DeviceInstance
        var deviceElements = projectXml
            .Descendants(ns + "Line")
            .Descendants(ns + "Segment")
            .Descendants(ns + "DeviceInstance")
            .Where(d => d.Attribute("Address") != null);

        if (!deviceElements.Any())
        {
            _logger.LogWarning("No devices found with Segment tag, trying direct DeviceInstance lookup");
